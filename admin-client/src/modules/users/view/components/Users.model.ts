...
class UsersModel extends ViewModel<UsersViewProps> {
  ...
  @Action()
  public handleFormFieldChange(field: keyof UserFormData, value: string): void {
    this.dataAccess.userFormData[field] = value;
  }
  ...
  public async handleCreateUser(): Promise<void> {
    const { username, email, password } = this.dataAccess.userFormData;
    await this.actions.createUser({ username, email, password });
